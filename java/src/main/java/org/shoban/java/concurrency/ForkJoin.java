package org.shoban.java.concurrency;
...
public class ForkJoin extends RecursiveTask<List<String>>
{
   ...
   public ForkJoin(String path, String extension)
   {
      this.path = path;
   ...
   protected List<String> compute()
   {
      ...
      List<ForkJoin> tasks = new ArrayList<ForkJoin>();
      ...
      if (content != null)
      {
         for (int i = 0; i < content.length; i++)
         {
            ...
            {
            	ForkJoin task = new ForkJoin(content[i].getAbsolutePath(), extension);
               task.fork();
               tasks.add(task);
            }
   ...
   private void addResultsFromTasks(List<String> list, List<ForkJoin> tasks)
   {
      for (ForkJoin item : tasks)
      {
